...
public class NegatorMenu extends AbstractContainerMenu
{
    // Menu, storing data
    private NegatorBlockEntity blockEntity;
    private Level level;
    private ContainerData data;
    public NegatorMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
        this(id, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(5));
    }

    public NegatorMenu(int id, Inventory inv, BlockEntity entity, ContainerData data)
    {
        super(ModMenuTypes.NEGATOR_MENU_TYPE.get(), id);
        checkContainerSize(inv, 0);
        blockEntity = (NegatorBlockEntity) entity;
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), pPlayer, ModBlocks.NEGATOR.get());
    }
}
